...
const login=async(credentials:ICredential)=>{
    try {
        ...
        const token = createToken({ id: user.getDataValue('id') });
        return {
            token,
            id
        }
...
const forgotPassword=async (email:string) => {try
    ...
    const name= user.getDataValue("username");
     ...
     `,`<a href=http://localhost:3001/auth/reset/${hashid}> Click here`)
    ...
    const changePassword=async(changedData:IReset)=>{
        try{
            ...
            const user:any=await userService.getUser(changedData.id)
       ...
       else
       {
        ...
        console.log(user.password,changedData.newPassword);
         ...
         await userRepo.updatedUser(user.getDataValue("password"),user.getDataValue("email"));
        
         sendEmail(user.getDataValue("email"), "Changed Password", `
         Hi, ${user.getDataValue("username")},
